...
# NIFTY 50 index instrument key for candles[web:181]
...
# def detect_trend(df):
#     """Return 1 for bullish, -1 for bearish, 0 for neutral/insufficient data."""
#     if df.empty or len(df) < 20:
#         return 0
#     ema5 = ema(df["close"], 5).iloc[-1]
#     ema20 = ema(df["close"], 20).iloc[-1]
#     print(f"EMA5={ema5:.2f}, EMA20={ema20:.2f}")
#     return 1 if ema5 > ema20 else -1

# VWAP DataFrame
def add_vwap(df):
    """Add running VWAP column to NIFTY candles."""
    if df.empty:
        return df
...
def find_vwap_cross_trigger(df):
    """
    Find last candle that crosses VWAP:
      - Bullish cross from below → trend = 1, trigger = candle high
      - Bearish cross from above → trend = -1, trigger = candle low
    Returns: (trend, trigger_price)
    """
...
def detect_trend_vwap(df):
    """
    Wrapper: add VWAP, then compute trend from VWAP cross.
    Keeps the same style as detect_trend() but returns 1/-1/0.
    """
    df = add_vwap(df)
    trend, trigger_price = find_vwap_cross_trigger(df)
    # store trigger on df if you want to inspect; main output is trend
    return trend, trigger_price

    
# trigger check helper
# This enforces “enter only when spot breaks that high/low.”

def should_enter_from_trigger(opt_type, trigger_price, current_spot):
    """
    CE: enter when spot >= trigger high
    PE: enter when spot <= trigger low
    """
...
def info_from_instrument_key(contracts, instrument_key: str):
    ...
    for c in contracts:
        if c.get("instrument_key") == instrument_key:
            return {
                "strike": c.get("strike_price"),
                "type": c.get("instrument_type"),   # CE / PE
                "expiry": c.get("expiry"),          # yyyy-mm-dd
                "trading_symbol": c.get("trading_symbol"),
            }
    return None
...
class Position:
    def __init__(self, entry_oid, instrument_key, qty, entry_price):
        ...
        # Log entry (paper or live) into CSVs
        row = [
            now_iso(),
            "BUY",
            TAG,
...
def dashboard():
    ...
    print("==========  NIFTY VWAP SCALPER  –", "PAPER" if PAPER else "LIVE", "MODE  ==========")
    ...
    for oid, pos in open_positions.items():
        ...
        if info:
            print(
                ...
                f"LTP {ltp:.2f}  "
                f"SL {pos.trail_price:.2f}  "   # this will move as trailing SL updates
                f"MTM {mtm:.0f}"
            )
        else:
            print(
...
# ======================================================================
def run_once():
    """VWAP cross logic → pick CE/PE → enter at trigger break."""
    df = get_nifty_intraday_candles(120)
    if df.empty:
        print("No candles, skipping.")
        return

    df = add_vwap(df)
    if trend == 0 or trigger_price is None:
        print("No VWAP cross setup, skipping.")
        return
...
def run_once_vwap():
    """Detect VWAP bias, pick CE/PE, place BUY + SL + TGT (one-shot)."""
    spot = get_nifty_ltp()
    ...
    trend = detect_trend_vwap(df)
    if trend == 0:
        print("No clear VWAP bias.")
        return
    ...
    print(f"VWAP BUY {opt_type} {strike} ({inst_key})")
    ...
    open_positions[entry_oid] = pos
    print("VWAP trade placed with SL/TGT and logged.")
...
def auto_loop(
    ...
    while True:
        ...
        # 2) Entry: only when no open positions AND trades left for the day
        if not open_positions and trades_done < max_trades_per_day:
            print("\n=== ENTERING NEW TRADE (VWAP CROSS) ===")
            run_once_vwap()       # <-- your VWAP cross + trigger + CE/PE entry

            # If an entry was actually taken, run_once_vwap() will have
            # added a Position to open_positions.
            if open_positions:
                trades_done += 1

        # 3) If there is an open position, just monitor it
        else:
            print("\n=== MONITORING POSITIONS ===")
            monitor_positions()

        # 4) Always show dashboard
        dashboard()

        # 5) Exit condition: after time window or max trades, once flat, stop
        now = dt.datetime.now().time()

...
def v2_indicators(df):
    ...
    # 2. CCI (20)
    df["cci"] = talib.CCI(
        df["ha_high"].values, df["ha_low"].values, df["ha_close"].values, 20
    )
...
def detect_ha_v2_trend(df):
    ...
    signals.append(
        1
        if latest["ha_close"] > latest["kc_upper"]
        else -1 if latest["ha_close"] < latest["kc_lower"] else 0
    )
    ...
    signals.append(
        1
        if adx_strong and latest["plus_di"] > latest["minus_di"]
        else -1 if adx_strong and latest["minus_di"] > latest["plus_di"] else 0
    )

    # 5. TSI
    signals.append(
        1
        if latest["tsi"] > 0 and latest["tsi"] > prev["tsi"]
...
def upstox_headers():
    ...
    }  # Fixed: removed deprecated Api-Version [web:23]
...
def hft_headers():
    return {
        **api_headers(),
        "Content-Type": "application/json",
    }  # Added missing hft_headers function

...
def v1_indicators(df):
    ...
    # 2. Williams %R (14)
    df['williams_r'] = talib.WILLR(df['ha_high'].values, df['ha_low'].values, df['ha_close'].values, 14)
    ...
    # 4. Aroon (14)
    df['aroon_up'], df['aroon_down'] = talib.AROON(df['ha_high'].values, df['ha_low'].values, 14)
    
    # 5. Stochastic (14,3,3)
    df['stoch_k'], df['stoch_d'] = talib.STOCH(df['ha_high'].values, df['ha_low'].values, df['ha_close'].values)
...
def detect_ha_v1_trend(df):
    ...
    # 2. Williams %R
    signals.append(1 if latest['williams_r'] > -20 else -1 if latest['williams_r'] < -80 else 0)
    
    # 3. Z-Score (reversal)
    signals.append(1 if latest['zscore'] < -1.0 else -1 if latest['zscore'] > 1.0 else 0)
    
    # 4. Aroon
    signals.append(1 if latest['aroon_up'] > 70 else -1 if latest['aroon_down'] > 70 else 0)
    ...
    print(f"🎯 V1 REVERSAL: Bull={bull}/5 Bear={bear}/5 | Z={latest['zscore']:.2f} %R={latest['williams_r']:.0f}")
...
def get_access_token():
    with open(ACCESS_TOKEN_FILE, "r") as f: return f.read().strip()
def api_headers(): 
...
def upstox_headers():
    return {**api_headers(), "Api-Version": "2.0", "Content-Type": "application/json"}
# ======================================================================
# LOGGING HELPERS
# ======================================================================
...
#========================================================================
def get_nifty_ltp():
    r = requests.get(f"{BASE_REST}/market-quote/ltp", headers=api_headers(), params={"instrument_key": "NSE_INDEX|Nifty 50"})
    r.raise_for_status()
    return r.json()["data"]["NSE_INDEX:Nifty 50"]["last_price"]
def get_nifty_intraday_candles(minutes_back):
    end_time, start_time = dt.datetime.now(), dt.datetime.now() - dt.timedelta(minutes=minutes_back)
    r = requests.get(f"https://api.upstox.com/v2/historical-candle/intraday/{NIFTY_SPOT_INSTRUMENT}/1minute", 
                    headers=upstox_headers(), params={"to_date": end_time.isoformat(timespec="seconds"), 
                                                      "from_date": start_time.isoformat(timespec="seconds")})
    r.raise_for_status()
    data = r.json()["data"]["candles"]
    return pd.DataFrame([{"time": dt.datetime.fromisoformat(c[0].replace("Z","+00:00")), 
                         "open":c[1],"high":c[2],"low":c[3],"close":c[4],"volume":c[5]} for c in data])
...
def dashboard(): # V1 version
    ...
    print(f"Open MTM: {sum((get_option_ltp(p.instrument_key)-p.entry_price)*p.qty for p in open_positions.values()):.0f}")
...
# AUTO LOOP
# ======================================================================       
def auto_loop(max_trades_per_day=3, monitor_interval_sec=30, start_time=dt.time(9, 15), end_time=dt.time(15, 15)):
    ...
    while True:
        ...
        # Stop conditions
        now = dt.datetime.now().time()
        if (now > end_time and not open_positions) or (trades_done >= max_trades_per_day and not open_positions):

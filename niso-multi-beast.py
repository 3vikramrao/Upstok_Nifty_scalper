...
def add_supertrend(df, atr_period=10, multiplier=3.0):
    ...
    for i in range(1, len(df)):
        ...
        if in_uptrend[-1]:
            ...
            if (supertrend[i] > supertrend[i - 1] and 
                not np.isnan(supertrend[i - 1])):
...
def detect_multi_trend(df):
    ...
    # 1. EMA Crossover
    ema_bull = (latest["ema9"] > latest["ema15"] and 
                prev["ema9"] <= prev["ema15"])
    if ema_bull:
        signals.append(1)
    elif (latest["ema9"] < latest["ema15"] and 
          prev["ema9"] >= prev["ema15"]):
        signals.append(-1)
    ...
    # 3. SuperTrend
    if (latest["close"] > latest["supertrend"] and 
        prev["close"] <= prev["supertrend"]):
        signals.append(1)
    elif (latest["close"] < latest["supertrend"] and 
          prev["close"] >= prev["supertrend"]):
        signals.append(-1)
    
    # 4. Parabolic SAR
    if (latest["close"] > latest["parabolic_sar"] and 
        prev["close"] <= prev["parabolic_sar"]):
        signals.append(1)
    elif (latest["close"] < latest["parabolic_sar"] and 
          prev["close"] >= prev["parabolic_sar"]):
        signals.append(-1)
...
def build_strike_maps(contracts):
    ...
    # Build strike maps
    ce_map = dict(zip(ce_df["strike_price"].astype(float), 
                     ce_df["instrument_key"]))
    pe_map = dict(zip(pe_df["strike_price"].astype(float), 
                     pe_df["instrument_key"]))
...
class Position:
    ...
    def __init__(self, entry_oid, instrument_key, qty, entry_price):
        ...
        print(f"Position: entry={entry_price:.2f}, "
...
def dashboard():
    ...
    print("🎯 HA-V2 TREND BREAKOUT BEAST –", 

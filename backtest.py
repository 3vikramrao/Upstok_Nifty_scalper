...
def load_strategy(strategy_name):
    ...
    if not strategy_path.exists():
        print("Available:", [f.stem for f in strategy_dir.glob("*.py") if f.stem != "__init__"])
        sys.exit(1)
...
def run_backtest(args):
    ...
    for symbol in symbols:
        nifty = yf.download(symbol, start=start_date, end=end_date, interval=args.interval, progress=False)
        if len(nifty) > 10:
            break
    ...
    # FIXED METRICS
    total_trades = len(nifty[nifty['Long_Signal'] == True]) + len(nifty[nifty['Short_Signal'] == True])
